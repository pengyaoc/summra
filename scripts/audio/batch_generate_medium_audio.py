...
Batch generate Gemini TTS audio for medium summaries

This script finds all books that don't have Gemini-generated medium summary audio
...
Usage:
    python scripts/audio/batch_generate_medium_audio.py
    python scripts/audio/batch_generate_medium_audio.py --dry-run
"""
...
def find_books_without_audio(db: Database) -> list:
    """Find all books that have medium summaries but no audio file on disk"""
    conn = db.get_connection()
    cursor = conn.cursor()

    # Query for books with medium summaries
    query = """
    SELECT
        b.id,
        b.title,
    ...
    for row in results:
        book_id = row[0]
        audio_path = audio_dir / f"book_{book_id}_medium_gemini.wav"
...
def generate_audio_for_book(db: Database, tts_handler: GeminiTTSHandler,
    ...
    """Generate audio for a single book's medium summary"""
    print(f"\n{'='*80}")
    ...
    summary = db.get_summary(book['book_id'], 'medium')
    if not summary:
        print(f"❌ No medium summary found for book ID {book['book_id']}")
        return False
    ...
    if dry_run:
        print(f"\n[DRY RUN] Would generate audio with ID: book_{book['book_id']}_medium")
        print(f"[DRY RUN] Cleaned text preview (first 500 chars):\n{cleaned_text[:500]}")
        return True

    # Generate audio
    audio_id = f"book_{book['book_id']}_medium"
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch generate Gemini TTS audio for medium summaries'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    print("  python scripts/audio/generate_gemini_audio_batch_offline.py --summary-type medium")
    print("="*80)
    print()
    print("="*80)
    print("Batch Generate Gemini TTS Audio for Medium Summaries")
    print("="*80)
    ...
    print("\nFinding books without Gemini medium summary audio...")
    books = find_books_without_audio(db)

    if not books:
        print("\n✅ All books with medium summaries already have audio!")
        return
