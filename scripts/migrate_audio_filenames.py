...
Usage:
    python scripts/migrate_audio_filenames.py          # Preview changes (dry run)
    python scripts/migrate_audio_filenames.py --apply  # Apply changes
"""
...
def migrate_files(dry_run=True):
    ...
    if dry_run:
        ...
        print(f"{'='*80}\n")
        print("To apply changes, run: python scripts/migrate_audio_filenames.py --apply")
        return
