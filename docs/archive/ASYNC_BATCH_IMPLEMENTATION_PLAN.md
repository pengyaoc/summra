...
1. **Test --list-jobs**: `python scripts/content/generate_summaries.py --list-jobs`
2. **Test with --sync flag**: `python scripts/content/generate_summaries.py /tmp/pg75011.txt --sync` (should work as before)
3. **Test async mode**: `python scripts/content/generate_summaries.py /tmp/pg75011.txt` (new async flow)
